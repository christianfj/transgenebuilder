...
# Get database
my $sequence_lib = retrieve('bin/GLODB');
...
#local $" = '';
#my @testarray = ("AAAAAAAA", "AAAA", "l", "g", "c", "t");
#my $test = \@testarray;
#my $pos = 0;
#print "@{$test}[$position..$position+1]";
#my $scoretest = $sequence_lib ->{ "@{$test}[$position..$position+1]"  };
...
my ($optimization_results, @refine_counter1) = OptimizerTools::optimize($sequence_lib, $AAseq);
...
#print "seq: @refine_counter1[10..13]";
#print "\n";
#my $position = 10;

#my $refrefine = @refine_counter1;

#my $words = join('', @refine_counter1[$position-3..$position]);
...
#  local $" = '';
#  #my @arr = (1, 2, 3);
#  $words = "@refine_counter1[10..13]"; # $scalar contains '1:2:3'
#}
